...
namespace _01_Mi_Primera_Vez.Presentacion
{
    public partial class CUPersonal : UserControl
    {
        ...
        {
            string busqueda = txtBuscar.Text.ToLower(); // Convertir a minúsculas para búsqueda insensible a mayúsculas

            // Obtener la lista de todos los registros
            var logicaPersonal = new cls_personal();
            var todosLosRegistros = logicaPersonal.todos(); // Asumiendo que este método devuelve una lista de dto_personal

            // Filtrar la lista de personas
            var resultados = todosLosRegistros
                .Where(p => p.nombre.ToLower().Contains(busqueda) || p.cedula.ToLower().Contains(busqueda))
                .ToList();

            // Mostrar los resultados en el DataGridView
            dataGridView1.DataSource = resultados; // Asegúrate de que dataGridView1 esté en tu formulario

            // Si no se encontraron resultados, puedes mostrar un mensaje
